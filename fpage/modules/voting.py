...
from fpage.models import ThreadVotes, CommentVotes, Submission, Comment, User
...
def vote():
    ...
    if vote_type == "comment":
        vote_object = Comment.query.filter_by(id=obj_id).first()
    elif vote_type == "submission":
        vote_object = Submission.query.filter_by(id=obj_id).first()
    else:
